...
struct PLIC : public sc_core::sc_module, public interrupt_gateway {
    ...
    void gateway_incoming_interrupt(uint32_t irq_id) {
        ...
        pending_interrupts |= 1 << irq_id; //< 将pending_interrupts[irq_id]比特位置高
    ...
    void transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
        if (trans.get_address() == Addr_Target_0_Claim_Response) {
            if (trans.get_command() == tlm::TLM_WRITE_COMMAND) {
                ...
                *((uint32_t*)trans.get_data_ptr()) = min_id;    // zero means no more interrupt to claim //< 在这里，ID越小，优先级越高；
            } else {
    ...
    void run() {
        while (true) {
            ...
            if (!eip) {
                ...
                if (irqs > 0) {
                    eip = true;
                    target_hart->trigger_external_interrupt();   //TODO: check for priority threshold and order by priorities //< 硬连线至mip
                }
            }
        }
    }
