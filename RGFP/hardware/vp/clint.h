...
struct CLINT : public clint_if,
    ...
    uint64_t update_and_get_mtime() override { // 用现在的时间计算出mtime寄存器的值
    ...
    void run() { //设置好cmp寄存器后，此进程被触发，之后再挂起等待定时时间后再次执行。
        while (true) {
    ...
    void transport(tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
        ...
        auto it = addr_to_reg.find(trans.get_address()); //要对mtime或cmptime寄存器中的一个进行操作
