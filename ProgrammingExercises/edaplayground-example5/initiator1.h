...
//Initiator writes to all 4 memories, and demonstrates DMI and debug transport 
	...
	void thread_process(){
		...
		for (int i = 0; i < RUN_LENGTH; i+=4)
		{
			...
			else
			{
				...
				// Initiator obliged to check reponse status
			...
			// Accumulate local time and sychronize when quantum is reached
			m_qk.set(delay);
